...
// Getter for flat_fee.
double OvernightPackage::get_flat_fee() const {
    return flat_fee;
}
...
// Make sure pointer is casted as OvernightPackage and not derived class Package.
std::ostream& operator<<(std::ostream &os, const OvernightPackage& p) {
    return os << "Sender:\n" << p.get_send_name() << "\n" << p.get_send_address() <<
